...
public class AdminDAOImpl implements AdminDAO {
	...
	public String districtMasterSave(DistrictForm districtForm) {
		...
		String district1 = "select * from district where upper(districtname) ='"
				+ districtForm.getDistrictName().replaceAll("%20", " ")
						.toUpperCase() + "'";
		// String sql =
		// "select s.stateId from district as d inner join state as s on s.stateid = d.stateid where "+
	...
	public String cityMasterSave(CityForm cityForm) {
		...
		District d = (District) session.load(District.class,
				cityForm.getDistrictId());
		City city = new City();
	...
	public String regionMasterSave(RegionForm regionForm) {
		...
		String sql = "select * from region where regionname = '"
				+ regionForm.getRegionName() + "' and districtid = '"
				+ regionForm.getDistrictId() + "' and status = '"
				+ regionForm.getStatus() + "'";
		Query query = session.createSQLQuery(sql);
		List l = query.list();
	...
	public List<CourseName> courseNameList() {
		Session session = sessionFactory.getCurrentSession();
		Query query = session
				.createQuery("from CourseName  where status = 'A'");
	...
	public String manageCourse(ManageCourse manageCourse) {
		...
		CourseType ct = (CourseType) session.load(CourseType.class,
				manageCourse.getCourseType());
		String coursetype = ct.getCourseType();
		CourseName courseName = new CourseName();
		courseName.setCourseduration(manageCourse.getDuration() == null ? ""
				: manageCourse.getDuration());
		courseName.setCoursename(manageCourse.getCourseName() == null ? ""
				: manageCourse.getCourseName());
		courseName.setCourseTypeS(ct);
		courseName.setStatus(manageCourse.getStatus() == null ? ""
				: manageCourse.getStatus());
		courseName.setPaidunpaid(manageCourse.getFreePaid() == null ? ""
				: manageCourse.getFreePaid());
		courseName
				.setModeOfTraining(manageCourse.getModeOfTraining() == null ? ""
						: manageCourse.getModeOfTraining());
		if (manageCourse != null && manageCourse.getCourseName() != null
				&& manageCourse.getCourseName().length() > 1
				&& coursetype != null && coursetype.length() > 1) {
			courseName.setCourseCode(ct.getCourseType().substring(0, 1)
					.toUpperCase()
					+ manageCourse.getCourseName().substring(0, 2)
							.toUpperCase()
					+ StringUtils.leftPad(String.valueOf(maxId), 3, "0"));
			courseName.setSeqNo(maxId);
		}
		if (manageCourse.getOnline() == null
				|| manageCourse.getOnline().equals("false")) {
			courseName.setOnline("Nil");
		} else {
			courseName.setOnline("Online");
		}
		if (manageCourse.getClassroom() == null
				|| manageCourse.getClassroom().equalsIgnoreCase("false")) {
		...
		String sqlInsert = "select ct.coursetype , cn.coursename "
				...
				+ " where cn.coursetypeid='" + manageCourse.getCourseType()
				+ "' and cn.coursename= '" + manageCourse.getCourseName() + "'";
	...
	public String manageTrainingPartnerSave(
			ManageTrainingPartnerForm manageTrainingPartnerForm) {
		Session session = sessionFactory.getCurrentSession();
		...
		try {
			...
			encryprPassword = encryptionPasswordANDVerification
					.encryptPass(passwordString);

		} catch (NoSuchAlgorithmException e) {
			new ZLogger("manageTrainingPartnerSave", "Exception while  "
					+ e.getMessage(), "AdminDAOImpl.java");
		}
		...
		String nextSequenceUserID = pageLoadService.getNextCombinationId(
				TPPrefix, "manageTrainingPartner", "00");
		...
		loginDetails.setIsActive(manageTrainingPartnerForm.getStatus()
				.equalsIgnoreCase("A") ? "Y" : "N");
		...
		manageTrainingPartner
				.setPAN(manageTrainingPartnerForm.getPAN() == null ? ""
						: manageTrainingPartnerForm.getPAN());
		manageTrainingPartner.setTrainingPartnerName(manageTrainingPartnerForm
				.getTrainingPartnerName() == null ? ""
				: manageTrainingPartnerForm.getTrainingPartnerName());
		manageTrainingPartner.setUserId(nextSequenceUserID == null ? ""
				: nextSequenceUserID);
		manageTrainingPartner.setWebsiteUrl(manageTrainingPartnerForm
				.getWebsiteUrl() == null ? "" : manageTrainingPartnerForm
				.getWebsiteUrl());
		manageTrainingPartner
				.setHeadOfficeDataAddress1(manageTrainingPartnerForm
						.getHeadOfficeDataAddress1() == null ? ""
						: manageTrainingPartnerForm.getHeadOfficeDataAddress1());
		manageTrainingPartner
				.setHeadOfficeDataAddress2(manageTrainingPartnerForm
						.getHeadOfficeDataAddress2() == null ? ""
						: manageTrainingPartnerForm.getHeadOfficeDataAddress2());
		manageTrainingPartner
				.setPin(manageTrainingPartnerForm.getPin() == null ? ""
						: manageTrainingPartnerForm.getPin());
		manageTrainingPartner.setDistrict(d);
		manageTrainingPartner.setCity(c);
		manageTrainingPartner.setState(s);
		manageTrainingPartner
				.setEmail(manageTrainingPartnerForm.getEmail() == null ? ""
						: manageTrainingPartnerForm.getEmail());
		manageTrainingPartner.setLoginDetails(loginDetails);
		session.save(manageTrainingPartner);

		new ZLogger("manageTrainingPartnerSave", "all insert done",
				"AdminDAOImpl.java");
	...
	public String manageAssessmentAgencySave(
			ManageAssessmentAgencyForm manageAssessmentAgencyForm) {
		Session session = sessionFactory.getCurrentSession();
		PasswordGenerator passwordGenerator = new PasswordGenerator(6);
		char[] pass = passwordGenerator.get();
		String passwordString = String.valueOf(pass);
		...
		try {
			...
			new ZLogger("manageAssessmentAgencySave",
					"Exception while manageAssessmentAgencySave "
							+ e.getMessage(), "AdminDAOImpl.java");
		}
		...
		String APPrefix = "AP"
				+ manageAssessmentAgencyForm.getAssessmentAgencyName()
						.toUpperCase().substring(0, 3);
		String nextSequenceUserID = pageLoadService.getNextCombinationId(
				APPrefix, "manageAssessmentAgency", "00");
		...
		loginDetails.setStatus("A");
		loginDetails.setIsActive(manageAssessmentAgencyForm.getStatus()
				.equalsIgnoreCase("A") ? "Y" : "N");
		manageAssessmentAgency
				.setAgencyUniqueID(nextSequenceUserID == null ? ""
						: nextSequenceUserID);
		manageAssessmentAgency
				.setPAN(manageAssessmentAgencyForm.getPAN() == null ? ""
						: manageAssessmentAgencyForm.getPAN());
		manageAssessmentAgency
				.setAssessmentAgencyName(manageAssessmentAgencyForm
						.getAssessmentAgencyName() == null ? ""
						: manageAssessmentAgencyForm.getAssessmentAgencyName());
		manageAssessmentAgency.setWebsiteUrl(manageAssessmentAgencyForm
				.getWebsiteUrl() == null ? "" : manageAssessmentAgencyForm
				.getWebsiteUrl());
		manageAssessmentAgency
				.setHeadOfficeDataAddress1(manageAssessmentAgencyForm
						.getHeadOfficeDataAddress1() == null ? ""
						: manageAssessmentAgencyForm
								.getHeadOfficeDataAddress1());
		manageAssessmentAgency
				.setHeadOfficeDataAddress2(manageAssessmentAgencyForm
						.getHeadOfficeDataAddress2() == null ? ""
						: manageAssessmentAgencyForm
								.getHeadOfficeDataAddress2());
		manageAssessmentAgency
				.setPin(manageAssessmentAgencyForm.getPin() == null ? ""
						: manageAssessmentAgencyForm.getPin());
		manageAssessmentAgency.setDistrict(d);
		manageAssessmentAgency.setCity(c);
		manageAssessmentAgency
				.setEmail(manageAssessmentAgencyForm.getEmail() == null ? ""
	...
	public List<PersonalInformationTrainee> traineeUserManagementSearch(
		...
		String like = " where upper(pitp.firstname) like '"
				+ FirstName.toUpperCase() + "' and pitp.MiddleName like '"
				+ MiddleName + "' and pitp.LastName like '" + LastName
				+ "' and " + "pitp.AadharNumber like '" + AadharNumber
		...
		String sql = "Select " + select
				+ "  from PersonalInformationTrainee as pitp " + join + like;
		System.out.println("sql " + sql);
		Query query = session.createSQLQuery(sql);
		List<PersonalInformationTrainee> list = query.list();
		new ZLogger("traineeUserManagementSearch", "list  " + list.toString(),
				"AdminDAOImpl.java");
		if (list.size() > 0) {
			return list;
		} else {
	...
	public List<PersonalInformationTrainer> trainerUserManagementSearch(
		...
		String sql = "Select " + select
				+ "  from PersonalInformationTrainer as pitp " + join + like;
		Query query = session.createSQLQuery(sql);
		List<PersonalInformationTrainer> list = query.list();
		new ZLogger("trainerUserManagementSearch", "list  " + list,
				"AdminDAOImpl.java");
		if (list.size() > 0) {
			return list;
		} else {
			new ZLogger("trainerUserManagementSearch", "list size null",
					"AdminDAOImpl.java");
			list = null;
			return list;
		}
	...
	public List<PersonalInformationAssessor> assessorUserManagementSearch(
			AssessorUserManagementForm assessorUserManagementForm,
		...
		if (profileid == 8) {
			...
			userBuffer.append(" AND pitp.assessmentagencyname="
					+ perAssessorAgencyID);
		}
		String join = " inner join loginDetails as ld on pitp.loginDetails = ld.id";
		String like = " where upper(pitp.FirstName) like '"
		...
		String sql = "Select " + select
				+ "  from PersonalInformationAssessor as pitp " + join + like;
		Query query = session.createSQLQuery(sql);
		List<PersonalInformationAssessor> list = query.list();
		new ZLogger("assessorUserManagementSearch", "list size " + list.size(),
				"AdminDAOImpl.java");
		if (list.size() > 0) {
			new ZLogger("assessorUserManagementSearch", "list size gt thaan 0",
					"AdminDAOImpl.java");
			return list;
		} else {
			new ZLogger("assessorUserManagementSearch", "list size null",
					"AdminDAOImpl.java");
			list = null;
			return list;
		}
	...
	public List<PersonalInformationTrainingInstitute> trainingCenterUserManagementSearch(
			TrainingCenterUserManagementForm trainingCenterUserManagementForm,
			Integer profileid, Integer userID) {
		Session session = sessionFactory.getCurrentSession();
		 ...
		 * userBuffer
		 * .append(" AND pitp.trainingpartnername="+perTrainingPartnerID); }
		 */
		String join = " inner join loginDetails as ld on pitp.loginDetails = ld.id";
		String like = " where upper(pitp.FirstName) like '"
				+ FirstName.toUpperCase() + "' and pitp.MiddleName like '"
				+ MiddleName + "' and pitp.LastName like '" + LastName
				+ "' and " + "pitp.PAN like '" + PanNumber
				+ "' and ld.status like '" + status + "'";
		like = like + userBuffer.toString();
		...
		String sql = "Select " + select
				+ "  from PersonalInformationTrainingInstitute as pitp " + join
				+ like;
		Query query = session.createSQLQuery(sql);
		List<PersonalInformationTrainingInstitute> list = query.list();
		if (list.size() > 0) {
			new ZLogger("trainingCenterUserManagementSearch",
					"list size gt thaan 0", "AdminDAOImpl.java");
			return list;
		} else {
			new ZLogger("trainingCenterUserManagementSearch", "list size null",
					"AdminDAOImpl.java");
			list = null;
			return list;
		}
	...
	public List<AdminUserManagement> adminUserManagementSearch() {
		new ZLogger("adminUserManagementSearch",
				"inside adminUserManagementSearch", "AdminDAOImpl.java");
		Session session = sessionFactory.getCurrentSession();
	...
	public String adminUserManagementSave(
			AdminUserManagementForm adminUserManagementForm) {
		new ZLogger("adminUserManagementSave",
				"inside adminUserManagementSave", "AdminDAOImpl.java");
		Session session = sessionFactory.getCurrentSession();
		...
		if (l != null && l.size() > 0) {
			...
			adminUserManagement.setAadharNumber(adminUserManagementForm
					.getAadharNumber());
			adminUserManagement.setFirstName(adminUserManagementForm
					.getFirstName());
			adminUserManagement.setLastName(adminUserManagementForm
					.getLastName());
			adminUserManagement.setMiddleName(adminUserManagementForm
					.getMiddleName());
			adminUserManagement.setLoginDetails(loginDetails);
			Integer adminUserManagementIdd = (Integer) session
					.save(adminUserManagement);
	...
	public String manageCourseContentSearch(
			ManageCourseContentForm manageCourseContentForm) {
	...
	public String assessorUserManagementSave(
		...
		if (l != null && l.size() > 0) {
			...
			assessorUserManagement.setAadharNumber(assessorUserManagementForm
					.getAadharNumber());
			assessorUserManagement.setFirstName(assessorUserManagementForm
					.getFirstName());
			assessorUserManagement.setLastName(assessorUserManagementForm
					.getLastName());
			assessorUserManagement.setMiddleName(assessorUserManagementForm
					.getMiddleName());
			assessorUserManagement.setLoginDetails(loginDetails);
			Integer assessorUserManagementIdd = (Integer) session
					.save(assessorUserManagement);
	...
	public String trainingCalendarForm(TrainingCalendarForm trainingCalendarForm) {
		...
		CourseName courseName = (CourseName) session.load(CourseName.class,
				trainingCalendarForm.getCourseName());
		if (courseName != null && courseName.getCourseCode() != null
				&& courseName.getCourseCode().length() > 1) {
			tc.setBatchCode(courseName.getCourseCode() + "/"
					+ StringUtils.leftPad(String.valueOf(maxId), 5, "0"));
			tc.setSeqNo(maxId);
		}
	...
	public String manageAssessmentQuestionsSave(
			AssessmentQuestionForm_old assessmentQuestionForm) {
		Session session = sessionFactory.getCurrentSession();
		...
		if (assessmentQuestionForm.getId() <= 0) {
			...
			assessmentQuestion = (AssessmentQuestion_old) session.load(
					AssessmentQuestion_old.class,
		...
		String where = " where coursetype = '"
				+ assessmentQuestionForm.getCourseTypeId()
				+ "' and coursename = '"
				+ assessmentQuestionForm.getCourseName()
				+ "' and questionTitle = '"
		...
		if (l != null && l.size() > 0 && assessmentQuestionForm.getId() <= 0) {
			...
			if (assessmentQuestionForm.getId() > 0) {
				...
				assessmentQuestionIdd = (Integer) session
	...
	@Override
	public boolean trainingPartnerPass(ChangePasswordForm changePasswordForm,
			String id) {
		String oldPassword = changePasswordForm.getOldPassword();
		String newPassword = changePasswordForm.getNewPassword();
	...
	@Override
	public String contactTrainigPartnerSave(ContactTrainee contactTrainee,
			String id) {
		...
		new ZLogger("contactTrainigPartnerSave",
				"sent mail to........................", "AdminDAOImpl.java");
		contactTrainerModel.setEmailAddress(email);
		contactTrainerModel.setMessageDetails(msg);
		contactTrainerModel.setUserId(id);
		Integer contactTrainerModelId = (Integer) session
				.save(contactTrainerModel);
		new ZLogger("contactTrainigPartnerSave",
				"contactTraineeSave after save", "AdminDAOImpl.java");
	...
	public String saveFeedbackMaster(FeedbackMaster feedbackMaster) {
		try {
			...
			new ZLogger("saveFeedbackMaster", "saveFeedbackMaster after save",
					"AdminDAOImpl.java");
			if (saveFeedbackMasterId != null
					&& saveFeedbackMasterId.intValue() > 0) {
	...
	@Override
	public List<TrainerAssessmentSearchForm> searchTrainerForAssessmentValidation(
			int courseNameId, int trainingPartnerId) {
		...
		if (courseNameId > 0 || trainingPartnerId > 0) {
			...
			if (trainingPartnerId > 0) {
				strQuery.append("pitp.personalinformationtrainingpartnerid ="
						+ trainingPartnerId);
			}
		}
	...
	public int getElegibilityForAssessment(int coursenameid) {
		Session session = sessionFactory.getCurrentSession();
		String sql = "select eligibility from assessmenteligibilitytrainer where coursenameid="
				+ coursenameid;
		Query query = session.createSQLQuery(sql);
	...
	@Override
	public int saveTrainerAssessment(
			TrainerAssessmentEvaluation trainerAssessmentEvaluation) {
		Session session = sessionFactory.getCurrentSession();
		Integer trainerAssessmentEvaluationId = (Integer) session
				.save(trainerAssessmentEvaluation);
	...
	public void updateUser(String userid, String tableName, String status) {
		Session session = sessionFactory.getCurrentSession();
		new ZLogger("contactTrainigPartnerSave", "update " + tableName
				+ " set isActive='" + status + "' where id=" + userid,
				"AdminDAOImpl.java");
		String sql = "update " + tableName + " set isActive='" + status
				+ "' where id=" + userid;
		Query query = session.createSQLQuery(sql);
		query.executeUpdate();
	...
	public List searchManageCourse(String data) {
		...
		if (!name.equalsIgnoreCase("ALL")) {
			...
			try {
				courseName = (totalConnected[1].split("="))[1].replaceAll(
						"%20", " ").trim();
			} catch (Exception e) {
				courseName = "%";
			}
		...
		if (!name.equalsIgnoreCase("ALL"))
			sql = "select cn.coursetypeid,ct.coursetype , cn.coursename , cn.courseduration , cn.paidunpaid ,  cn.status ,cn.coursenameid , cn.online , cn.classroom"
					...
					+ " where cast(cn.coursetypeid as varchar(10)) like '"
					+ courseType
					+ "%' and upper(cn.coursename) like '"
					+ courseName.toUpperCase()
					+ "%'"
					+ "  and paidunpaid like'"
					+ freePaid
					+ "%' and cn.courseduration like '"
					+ duration
					+ "%' and cn.status like '"
					+ status
	...
	public String editManageCourseData(String data) {
		...
		courseName = (totalConnected[1].split("="))[1].replaceAll("%20", " ")
				.trim();
		courseDuration = (totalConnected[4].split("="))[1].replaceAll("%20",
				" ").trim();
		...
		System.out.println(courseName + " " + courseDuration + " " + online
				+ "  " + classroom + " " + status + "  " + id);

		Session session = sessionFactory.getCurrentSession();
		CourseName courseNameee = (CourseName) session.load(CourseName.class,
				Integer.parseInt(id));
	...
	public String editState(String data) {
		...
		System.out
				.println(Integer.parseInt(id) + "  " + status + "   " + state);
		String newList = null;
		Session session = sessionFactory.getCurrentSession();
		State stateNameee = (State) session.load(State.class,
				Integer.parseInt(id));
	...
	public String CheckState(String data) {
		...
		System.out.println("passing name state update  :" + name);
		String sql = "select * from State where upper(stateName) like '"
				+ name.replaceAll("%20", " ").toUpperCase() + "%'";
		Session session = sessionFactory.getCurrentSession();
		Query query = session.createSQLQuery(sql);
	...
	public List<State> searchState(String data) {
		...
		Query query = session.createQuery("from State where statename like '"
				+ stateName + "%'");
	...
	public List onLoadDistrict(String data) {
		...
		System.out.println("state 1");
		Query query = session
				.createSQLQuery("select s.statename , d.districtName , d.status , d.districtId from district as d inner join state as s on s.stateid = d.stateid");
		List list = query.list();
		return list;
	...
	public List searchDistrict(String data) {
		...
		System.out.println("stateId " + stateId + " districtName "
				+ districtName);
		Session session = sessionFactory.getCurrentSession();
		String sql = "select  s.stateName , d.districtName  , d.districtId  from district as d inner join state as s on s.stateid = d.stateid where "
				+ " d.status = 'A' and upper(districtname) like '"
				+ districtName.replaceAll("%20", " ").toUpperCase()
				+ "%' and cast(s.stateid as varchar(100)) like '"
				+ stateId
				+ "%' ";
		Query query = session.createSQLQuery(sql);
		List list = query.list();
	...
	public List searchCity(String data) {
		...
		String sql = "select s.statename , d.districtname , c.cityname , c.status , c.cityId,d.districtid  ,s.stateid from city as c  "
				...
				+ " where CAST(s.stateid AS varchar(10)) like'"
				+ stateId
				+ "'"
				+ " and c.cityName like '"
				+ cityName
				+ "%' and  CAST(d.districtid AS varchar(10)) like '"
				+ districtId + "'";
	...
	public List onLoadRegion(String data) {
		...
		Query query = session
				.createSQLQuery("select d.districtName , r.regionname ,r.id , s.statename , c.cityname ,r.cityid , r.districtid , r.stateid , r.status  from region r left join city c on c.cityid = r.cityid left join district d on d.districtid = r.districtid left join state s on s.stateid = r.stateid ");
		List list = query.list();
		return list;
	...
	public String editRegionData(String data) {
		...
		System.out.println("checkkk data==>" + regionId + regionName + stateId
				+ districtId + cityId);
		Session session = sessionFactory.getCurrentSession();
	...
	public List traineeAssessmentCalender(String data) {
		...
		String sql = "select B.coursetype,C.coursename,A.trainername,A.assessmentdate,A.assessmenttime,D.firstname || D.middlename || D.lastname,A.trainingcalendarid,A.assessor  from trainingcalendar A  "
				...
				+ " where  cast(B.coursetypeid as varchar(10)) like '"
				+ courseType
				+ "%'  and cast(C.coursenameid as varchar(10)) like  '"
				+ courseName
				+ "%' and (D.firstname || ' '|| D.middlename ||' '|| D.lastname)  like  '"
				+ trainerName
				+ "%'  and  cast(A.assessmentdate as varchar(10)) like  '"
				+ assDate
				+ "%'  and cast(assessmenttime as varchar(10)) like '"
				+ assTime + "%'  ";
	...
	public List getQuestions(String data) {
		...
		int courseNameSearch = Integer
				.parseInt((totalConnected[0].split("="))[1]);
		int courseTypeSearch = Integer
				.parseInt((totalConnected[1].split("="))[1]);
		...
		System.out.println("contentLocationInput  " + courseNameSearch + "  "
				+ courseNameSearch1);
		System.out.println("courseTypeInput   " + courseTypeSearch + "  "
				+ courseTypeSearch1);
	...
	public List searchFeedbackMaster(String data) {
		...
		if (data.equalsIgnoreCase("ALL"))
			...
			sql = "select feedbacktypeid,coursetype,catogery,feedback,status from feedbackmaster"
					+ " where cast (coursetype as varchar(20)) like '"
					+ courseType
					+ "%' and cast(catogery as varchar(20)) like  '"
					+ catagory
					+ "%' and cast(feedback as varchar(20)) like '"
					+ feedback
					+ "%' and cast(status as varchar(10)) like '"
					+ status
					+ "%'";
		Query query = session.createSQLQuery(sql);
		List list = query.list();
	...
	public List searchAssessorDetail(String data) {
		...
		String sql = "select  maa.assessmentagencyname , concat( pia.firstname ,'' ,pia.middlename ,' '  , pia.lastname) , ld.loginid , "
				...
				+ " where maa.manageassessmentagencyid = '"
				+ Integer.parseInt(id) + "' AND ld.status='I'";
		Query query = session.createSQLQuery(sql);
		List list = query.list();
		System.out.println(list.size());
		return list;
	...
	public String changeAssessor(String data) {
		...
		if (personalInformationAssessor.getLoginDetails() != null) {
			String updateQry = "update logindetails set status ='" + newStatus
					+ "' where id ="
	...
	public void removeHolidayMaster(int id) {
		// TODO Auto-generated method stub
		Session session = this.sessionFactory.getCurrentSession();
		HolidayMaster p = (HolidayMaster) session.load(HolidayMaster.class,
				new Integer(id));
		if (null != p) {
			session.delete(p);
		}
	...
	public HolidayMaster getHolidayMasterById(int id) {
		...
		Query query = session.createQuery("from HolidayMaster where holidayId="
				+ id);
	...
	public List<HolidayMaster> listHolidayMaster() {
		...
		Session session = this.sessionFactory.getCurrentSession();
		List<HolidayMaster> mccList = session.createQuery("from HolidayMaster")
				.list();
	...
	public void addUnitMaster(UnitMaster p) {
		...
		List list = maxIDList.list();
		System.out.println(list.size());
		new ZLogger("UnitMaster", "list.size() " + list.size(),
				"AdminDAOImpl.java");
		if (list.size() > 0) {
			maxId = (int) list.get(0);
			// eligible = (String) list.get(0);
		...
		System.out.println(p.getUnitName().substring(0, 2).toUpperCase()
				+ StringUtils.leftPad(String.valueOf(maxId), 3, "0"));

		p.setUnitCode(p.getUnitName().substring(0, 2).toUpperCase()
				+ StringUtils.leftPad(String.valueOf(maxId), 3, "0"));
		p.setSeqNo(maxId);
		session.persist(p);
	}
	...
	public void removeUnitMaster(int id) {
		// TODO Auto-generated method stub
		Session session = this.sessionFactory.getCurrentSession();
		UnitMaster p = (UnitMaster) session.load(UnitMaster.class, new Integer(
				id));
		if (null != p) {
			session.delete(p);
		}
	...
	public List<UnitMaster> listUnitMaster() {
		...
		Session session = this.sessionFactory.getCurrentSession();
		List<UnitMaster> mccList = session.createQuery("from UnitMaster")
				.list();
	...
	public void addModuleMaster(ModuleMaster p) {
		...
		System.out.println("ModuleMaster " + p.getModuleId()
				+ " p.getUnitMaster() " + p.getUnitMaster().getUnitId());

		UnitMaster um = getUnitMasterById(p.getUnitMaster().getUnitId());
		System.out.println(p.getUnitMaster().getUnitName().substring(0, 2)
				.toUpperCase()
				+ p.getModuleName().substring(0, 2)
				+ StringUtils.leftPad(String.valueOf(maxId), 2, "0"));
		p.setModuleCode(p.getUnitMaster().getUnitName().substring(0, 2)
				.toUpperCase()
				+ p.getModuleName().toUpperCase().substring(0, 2)
				+ StringUtils.leftPad(String.valueOf(maxId), 2, "0"));
		p.setSeqNo(maxId);
		p.setUnitMaster(um);
	...
	public void removeModuleMaster(int id) {
		// TODO Auto-generated method stub
		Session session = this.sessionFactory.getCurrentSession();
		ModuleMaster p = (ModuleMaster) session.load(ModuleMaster.class,
				new Integer(id));
		if (null != p) {
			session.delete(p);
		}
	...
	public ModuleMaster getModuleMasterById(int id) {
		...
		Query query = session.createQuery("from ModuleMaster where ModuleId="
				+ id);
	...
	public List<ModuleMaster> listModuleMaster() {
		...
		Session session = this.sessionFactory.getCurrentSession();
		List<ModuleMaster> mccList = session.createQuery("from ModuleMaster")
				.list();
	...
	public void addSubjectMaster(SubjectMaster p) {
		// TODO Auto-generated method stub
		System.out.println("SubjectMaster " + p.getSubjectId() + " "
				+ p.getSubjectName());
		// getModuleMasterById
		Session session = this.sessionFactory.getCurrentSession();
		session.persist(p);
	...
	public void removeSubjectMaster(int id) {
		// TODO Auto-generated method stub
		Session session = this.sessionFactory.getCurrentSession();
		SubjectMaster p = (SubjectMaster) session.load(SubjectMaster.class,
				new Integer(id));
		if (null != p) {
			session.delete(p);
		}
	...
	public SubjectMaster getSubjectMasterById(int id) {
		...
		Query query = session.createQuery("from SubjectMaster where SubjectId="
				+ id);
	...
	public List<SubjectMaster> listSubjectMaster() {
		...
		Session session = this.sessionFactory.getCurrentSession();
		List<SubjectMaster> mccList = session.createQuery("from SubjectMaster")
				.list();
	...
	public void addTrainingSchedule(TrainingSchedule p) {
		...
		new ZLogger("TrainingSchedule", "list.size() " + list.size(),
				"AdminDAOImpl.java");
		...
		System.out.println("B" + um.getUnitName().substring(0, 2).toUpperCase()
				+ mm.getModuleName().substring(0, 1)
				+ StringUtils.leftPad(String.valueOf(maxId), 2, "0"));
		p.setBatchCode("B" + um.getUnitName().substring(0, 2).toUpperCase()
				+ mm.getModuleName().toUpperCase().substring(0, 1)
				+ StringUtils.leftPad(String.valueOf(maxId), 2, "0"));
		p.setSeqNo(maxId);
		// p.setUnitMaster(um);
...
System.out.println("........................."+assesQuestionForm.getModuleCode() );
	...
	public List<HolidayMaster> HolidayMastersearch(String data) {
		// TODO Auto-generated method stub
		System.out.println("inside dao holidaymaster " + data);
		Session session = this.sessionFactory.getCurrentSession();

		String[] arrData = data.split("@");
		System.out.println("inside dao holidaymaster " + data);
		// List<String> HolidayMaster = new ArrayList<String>();
		String Hdate = arrData[0];
		String HReason = arrData[1];

		System.out.println("date: " + Hdate + "  reason " + HReason);
		...
		String like = "where upper(hm.holidayReason) like '"
				+ HReason.toUpperCase() + "' and hm.holidayDate like '" + Hdate
				+ "'";

		String select = "hm.holidayId,hm.holidayDate,hm.holidayReason";

		String sql = "Select " + select + "  from HolidayMaster as hm " + like;
		System.out.println("sql " + sql);
		Query query = session.createSQLQuery(sql);
		List<HolidayMaster> list = query.list();
		new ZLogger("HolidayMastersearch", "list  " + list.toString(),
				"AdminDAOImpl.java");
		if (list.size() > 0) {
			return list;
		} else {
	...
	public List<TrainingPartner> TrainingPartnershowdetails(String data) {
		// TODO Auto-generated method stub
		System.out.println("inside dao TrainingPartnershowdetails " + data);
		Session session = this.sessionFactory.getCurrentSession();
		...
		String Link = arrData[1];

		System.out.println("date: " + Name + "  reason " + Link);
		...
		String like = "where upper(tp.trainingPartnerName) like '"
				+ Name.toUpperCase() + "' and tp.tpWebLink like '" + Link + "'";

		String select = "tp.trainingPartnerName,tp.tpWebLink,tp.trainingPartnerId,tp.status";

		String sql = "Select " + select + "  from TrainingPartner as tp "
				+ like;
		System.out.println("sql " + sql);
		Query query = session.createSQLQuery(sql);
		List<TrainingPartner> list = query.list();
		new ZLogger("TrainingPartnershowdetails", "list  " + list.toString(),
				"AdminDAOImpl.java");
		if (list.size() > 0) {
			return list;
		} else {
	...
	public List<StateMaster> StateMastershowdetails(String data) {
		// TODO Auto-generated method stub
		System.out.println("inside dao StateMastershowdetails " + data);
		Session session = this.sessionFactory.getCurrentSession();

		String[] arrData = data.split("@");
		System.out.println("inside dao StateMastershowdetails " + data);
		// List<String> HolidayMaster = new ArrayList<String>();
		String state = arrData[0];
		// String Link=arrData[1];
		...
		String like = "where upper(sm.stateName) like '" + state.toUpperCase()
				+ "'";

		String select = "sm.stateName,sm.stateId,sm.status";

		String sql = "Select " + select + "  from stateMaster as sm " + like;
		System.out.println("sql " + sql);
		Query query = session.createSQLQuery(sql);
		List<StateMaster> list = query.list();
		new ZLogger("StateMastershowdetails", "list  " + list.toString(),
				"AdminDAOImpl.java");
		if (list.size() > 0) {
			return list;
		} else {
